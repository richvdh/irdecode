...
class PulseDecoder:
    ...
    def pulse(self, pulse_width, space_width):
        if (pulse_width > 550 and pulse_width < 750 and
            (space_width is None or space_width > 10000)):
            self.logger.info("gap %s us", space_width)
            self.bitDecoder.gap()
            return

        if (pulse_width > 4400 and pulse_width < 4600 and
            space_width > 4400 and space_width < 4600):
            self.logger.debug("header")
            return

        if (pulse_width > 520 and pulse_width < 750 and
            space_width > 1500 and space_width < 1700):
            self.logger.debug("one")
            self.bitDecoder.bit(1)
            return

        if (pulse_width > 520 and pulse_width < 750 and
            space_width > 350 and space_width < 570):
